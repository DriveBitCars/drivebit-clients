...
После выкладки нового фронта при использовании кэша nginx для Prerender может понадобиться очистка. В CI ([`.github/workflows/deploy.yml`](../../.github/workflows/deploy.yml)) после проверки `index.html` выполняется скрипт [`scripts/warm-prerender-after-deploy.sh`](../../scripts/warm-prerender-after-deploy.sh):

1. `docker compose restart prerender` в `PRERENDER_COMPOSE_DIR` (по умолчанию `/opt/drivebit-prerender`).
