...
# Скрипт для настройки nginx проксирования аватаров
# Использование: ./setup-avatar-proxy.sh [SSH_USER@SSH_HOST]
# Пример: ./setup-avatar-proxy.sh root@143.198.69.242
...
# Проверяем, есть ли уже блок для /avatar/
if grep -q "location /avatar/" "$NGINX_CONFIG"; then
    echo "⚠️  Блок location /avatar/ уже существует в конфигурации"
    echo "📄 Текущий блок:"
    grep -A 10 "location /avatar/" "$NGINX_CONFIG" || true
