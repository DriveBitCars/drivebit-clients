...
private fun InspectionActReadyContent(
    ...
    val carInputId = remember { "inspection-act-photos-car-${kotlin.random.Random.nextInt()}" }
    val dashboardInputId = remember { "inspection-act-photos-dash-${kotlin.random.Random.nextInt()}" }
    ...
    Column(gap = 16.px, modifier = { width(100.percent) }) {
        ...
        if (otherPhotos.isNotEmpty()) {
            InspectionActPhotoSection(
                ...
                emptyText = "",
                photos = otherPhotos,
                kind = InspectionPhotoKind.Other,
